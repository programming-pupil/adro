...
func (s *Store) Append(ctx context.Context, request eventstore.AppendRequest) (eventstore.AppendResult, error) {
	...
	requestDigest, tenantID, workspaceID, err := validateAppend(request)
	if err != nil {
		return eventstore.AppendResult{}, err
	}
	...
	if replay, found, err := findReplay(ctx, tx.conn, request, requestDigest); err != nil {
		return eventstore.AppendResult{}, err
	} else if found {
	...
	if exists && (existingTenant != tenantID || existingWorkspace != workspaceID) {
	...
	if request.Lease != nil {
		if err := s.assertLease(ctx, tx.conn, tenantID, request.StreamID, *request.Lease); err != nil {
			return eventstore.AppendResult{}, err
		}
	}
	...
	for index, uncommitted := range request.Events {
		envelope, err := event.Commit(uncommitted, event.CommitMetadata{
			...
			EventID:  s.ids.NewID("event"), CommittedAt: committedAt, PreviousDigest: previousDigest,
		})
		if err != nil {
			return eventstore.AppendResult{}, fmt.Errorf("commit event %d: %w", index, err)
		}
	...
	if request.Snapshot != nil {
		...
		if digestBytes(request.Snapshot.Payload) != request.Snapshot.Digest {
	...
	if !exists {
		if _, err := tx.conn.ExecContext(ctx, `INSERT INTO event_streams
			...
			VALUES (?, ?, ?, 0, '', ?)`, request.StreamID, tenantID, workspaceID, nowMicros); err != nil {
	...
	for _, envelope := range envelopes {
		...
		if _, err := tx.conn.ExecContext(ctx, `INSERT INTO event_records
			...
			envelope.IdempotencyKey, requestDigest, envelope.PayloadDigest, envelope.PreviousDigest,
	...
	if request.Snapshot != nil {
		if _, err := tx.conn.ExecContext(ctx, `INSERT INTO event_snapshots
			...
			tenantID, request.StreamID, request.Snapshot.Sequence, request.Snapshot.Digest,
	...
	for index, message := range request.Outbox {
		if _, err := tx.conn.ExecContext(ctx, `INSERT INTO event_outbox
			...
			VALUES (?, ?, ?, ?, ?, ?, ?, ?)`, tenantID, request.StreamID, lastSequence, index,
	...
	result, err := tx.conn.ExecContext(ctx, `UPDATE event_streams
		...
		lastSequence, previousDigest, nowMicros, request.StreamID, tenantID, request.ExpectedSequence)
	if err != nil {
...
func validateAppend(request eventstore.AppendRequest) (string, string, string, error) {
	if request.ExpectedSequence < 0 || len(request.Events) == 0 {
		return "", "", "", errors.New("non-negative expected sequence and at least one event are required")
	}
	...
	if err := validateScope(tenantID, request.StreamID); err != nil {
		return "", "", "", err
	}
	...
	for index, item := range request.Events {
		if item.StreamID != request.StreamID || item.TenantID != tenantID || item.WorkspaceID != workspaceID {
			return "", "", "", fmt.Errorf("event %d does not match append stream scope", index)
		}
		if strings.TrimSpace(item.IdempotencyKey) == "" {
			return "", "", "", fmt.Errorf("event %d idempotency key is required", index)
		}
		if _, duplicate := seenKeys[item.IdempotencyKey]; duplicate {
			return "", "", "", fmt.Errorf("event %d repeats an idempotency key", index)
		}
		seenKeys[item.IdempotencyKey] = struct{}{}
		payload, err := coreencoding.Canonicalize(item.Payload)
		if err != nil {
			return "", "", "", fmt.Errorf("canonicalize event %d payload: %w", index, err)
		}
	...
	for index, message := range request.Outbox {
		if strings.TrimSpace(message.Topic) == "" || strings.TrimSpace(message.Key) == "" {
			return "", "", "", fmt.Errorf("outbox message %d topic and key are required", index)
		}
	}
	digest, err := coreencoding.Digest(identity)
	return digest, tenantID, workspaceID, err
}
