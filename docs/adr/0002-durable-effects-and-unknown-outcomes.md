...
Write tools use explicit `intent -> dispatch prepared -> dispatched -> receipt` facts. A crash, timeout, or lost response after durable dispatch and before receipt produces `outcome unknown`. The intent freezes a versioned reconciliation policy; only a policy-valid `effect.reconciled` transition can close the unknown state.
...
- `query`, `compensate`, `human_decision`, and `unrecoverable` are explicit policy classes. A missing write policy fails closed; a reconciled effect and its model-visible tool terminal fact commit atomically and are idempotent by decision.
