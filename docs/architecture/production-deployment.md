...
| Harness transcript/checkpoints | atomic JSON snapshot plus fsynced append-only transcript and crash-window journal | PostgreSQL `harness_sessions`/`session_transcript`/`session_checkpoints` with hash, sequence, and archive recall verification | block production if the selected adapter cannot validate the chain |
