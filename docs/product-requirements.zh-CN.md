...
版本：1.0（最终需求基线）  
状态：最终需求基线（待产品、研发、测试和安全签字确认）  
适用范围：ADRO 独立开源版、企业单租户部署、多租户 SaaS/私有化演进  
更新时间：2026-08-30

> 本文定义 ADRO 要解决的问题、必须交付的能力和明确不做的事情。Multica 仅作为公开行为和产品能力的参考对象；ADRO 不复制 Multica 的源代码、目录结构、UI、文案或内部数据模型，也不要求生产环境安装 Multica。
...
根据对公开 Multica 能力的梳理，以下能力值得参考：

- 自定义 Agent、Runtime、模型、指令、Skill、MCP 和并发上限。
- Project 与多个 GitHub repository/local directory 资源的绑定。
- Issue、评论、状态、附件、父子任务和运行历史。
- Squad 的角色路由、Autopilot 的定时/Webhook/手动触发。
- Daemon/Runtime 对本地 CLI、Worktree 和工具命令的执行。
- WebSocket 运行消息、Token usage、插件和扩展能力。

Multica 的不足或不适合直接继承的部分：

- 任务经常需要人工审核或手工重新执行，不能满足“提交后尽量无人值守”。
- 父子任务适合通用协作，但不等于产品、研发、测试、修复的业务状态机。
- 运行状态、代码提交、CI 结论和 PR 证据没有统一成为 ADRO 的业务事实。
- 通用平台的 UI、导航和对象数量较多，不适合以交付结果为中心的企业工作台。
- 直接绑定上游私有接口会让 ADRO 随 Multica 升级而被动变化。

ADRO 因此采用“参考能力、重新设计体验、隔离 Provider”的原则。
...
- GitHub 代码和 CI 证据适配器，包含 Commit、checks、PR、Webhook 和权限校验。
- 可审计 Artifact、EvidenceBundle、运行日志、测试报告和下载附件。
- 自有 Web 工作台，提供提交、监控、审核例外、复盘和报告查看。
- 钉钉、飞书两种企业应用的消息通知和交互回执，其他渠道通过同一通知 SPI 扩展。
...
- 不复制或嵌入 Multica 的 Web、桌面、移动 UI、聊天、看板、Inbox 和文案。
- 不实现 Multica 的 daemon、原生数据库、未版本化 API 兼容层，也不修改或 fork Multica 源码。
- 不把 Multica 父子任务树作为 ADRO 的核心数据模型；ADRO 使用阶段、Attempt、Transition、Dependency 和 Session。
- 不在首期承诺支持 Multica 的全部客户端、全部模型和全部外部频道；新增能力必须通过插件或 SPI。
- 不训练大模型、不替代 Git 托管、CI、测试环境或企业 IM；ADRO 编排并核验这些外部系统。
- 不允许无限自动修复、不把 Agent 自报结果当作唯一证据、不伪造 Git Author 或测试结论。
- 不承诺页面、菜单、生态和商业功能与 Multica 完全同等；目标是交付闭环和企业所需控制能力达到验收标准。
...
为覆盖 Multica 的 Squad/团队路由能力，ADRO 提供自己的 AgentTeam，而不是复制父子任务树：
...
UI 不得要求用户跳转 Multica 页面才能完成 ADRO 的主要流程。实时页面支持 cursor 断线恢复；没有权限的对象不出现在列表、搜索和事件流中。桌面宽屏显示完整侧栏，移动端优先显示工作台、需求、缺陷、例外和运行详情；钉钉/飞书内嵌页面复用同一权限和 API（P1）。
...
以下能力不是简单复刻 Multica，而是 ADRO 的差异化重点：
...
- 不复制 Multica 源文件、目录、变量命名、UI、图片、文案或私有表结构；保留必要的第三方许可证和归属说明。
- Provider 适配器和业务核心分离，生产版不需要安装任何外部编排产品；当前版本的本地执行器可独立替换。
- 任何“参考 Multica”都必须在设计评审记录中说明：参考的公开能力、ADRO 的差异、为什么不直接复用实现。
...
| Multica 核心能力（Project/Resource、Issue、Squad、Autopilot、Runtime、MCP/Skill、Attachment、Usage） | 5、6、7.5、8.3、10、12～14、17.1 | 已覆盖为 ADRO 自有对象/菜单/契约；只保留交付目标，不复制上游内部实现 |
| 记忆、编排、重试、恢复 | 8、9、16 | 已覆盖；状态机、ContextManifest、lease、幂等和故障恢复均为核心 |
| 钉钉、飞书能力 | 3.1、12、17-P0、18-H | 已覆盖；两者均为 P0，包含通知、卡片操作、验签和回执 |
| 数据分析流程 | 6.1、7.4、11、18-G | 已覆盖；由 Analyst Agent 执行，数据通过知识库/Skill，不把数据源做成核心菜单 |
| 自有 UI，解决 Multica UI 不顺手 | 3.3、13 | 已覆盖；18 个 ADRO 菜单和交互职责已列出，不嵌入 Multica UI |
| 完全抛弃 Multica、可开源 | 1.1、3.3、19 | 已覆盖；Provider 可拔出，不改/不 fork 上游，采用 clean-room 原则 |
| 通过真实证据达到推广地步 | 10、17、18、19 | 已覆盖；Commit、CI、部署、测试、权限、通知和故障注入是发布门槛 |

### 21.1 未列为核心的内容

没有发现会影响上述闭环的核心缺项。以下内容有意不放入 P0，不属于当前目标缺失：

- Multica 原生页面、父子任务内部语义、daemon 和数据库兼容层；
...
ADRO 不是 Multica 的 UI 改版，也不是把所有 Multica 外围功能重新做一遍。它是一个以 Agent 为执行主体、以员工为治理主体、以项目和租户为边界、以 Session 和证据为事实的自动交付系统：提交需求或 Bug 后，系统自动完成后续研发/分析和测试流程，只有无法安全自动处理的事项才打扰员工。

本节通过评审后，本文 `v1.0` 作为最终需求基线；后续代码实现和测试不得擅自缩减 P0，新增能力必须经过版本化需求变更。

## 22. 参考资料

- `ADRO-production-blueprint.zh-CN.md`：ADRO 架构基线、Provider、Runner、Artifact、Evidence 和生产部署边界。
- `docs/architecture/ga-readiness.md`：实现、参考实现和阻塞项的发布门禁。
- Multica 公开仓库和文档：Agent、Project/Resource、Issue、Runtime/Daemon、Squad、Autopilot、MCP/Skill、Attachment、WebSocket 和 Usage 等能力的黑盒参考。
