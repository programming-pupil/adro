...
| P0-CORE-010 | partial | `core/event.ValidateChain` and `core/reducer.ReplayVerified` produce verified replay/state-digest evidence; `internal/projection.Worker` now replays one tenant/stream partition from sequence zero with a digest-checked offset, while other views and full transaction composition remain pending |
...
| P0-STORE-001 | partial | EventStore, Snapshot, Lease, Blob, Secret, Scope, and Projection ports are independent; shared projection contract plus memory, SQLite, and PostgreSQL adapters now enforce digest/CAS/tenant rules, and `internal/projection.Worker` persists digest-checked offsets; full runtime composition and production SecretStore wiring remain pending |
