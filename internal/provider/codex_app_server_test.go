...
func TestExecuteCodexAppServerRetriesCompletelyEmptyTurn(t *testing.T) {
	...
	pid, output, err := executeCodexAppServer(ctx, executable, nil, "return evidence", t.TempDir(), "", false, "", "", "", nil)
	if err != nil {
		t.Fatal(err)
	}
	text := string(output)
...
func TestExecuteCodexAppServerFailsAfterEmptyTurnRetryLimit(t *testing.T) {
	...
	_, output, err := executeCodexAppServer(ctx, executable, nil, "return evidence", t.TempDir(), "", false, "", "", "", nil)
