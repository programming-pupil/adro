...
# Shared setup for real Codex acceptance suites. A parent automation runtime
# may set a private CODEX_HOME for itself; operators can explicitly
...
write_minimal_codex_config() {
  ...
  awk '
    BEGIN {
      # Keep managed root-level settings before every TOML table. Appending
      # sandbox_mode after [model_providers.custom] makes it a child of that
      # table, so Codex silently ignores the policy. Keep this managed block
      # at the top so it remains a root-level setting.
      print "# BEGIN adro-managed (do not edit; regenerated by real runner)"
      print "sandbox_mode = \"danger-full-access\""
      print "# END adro-managed"
      print ""
      section = ""
    }
    /^[[:space:]]*\[/ {
  ...
  # This is a table, so append it after the projected root-level settings and
  # provider table. Putting it before model_provider would re-scope those
  # source fields under shell_environment_policy in TOML.
...
write_real_codex_wrapper() {
  ...
  go_root_quoted="$(printf '%q' "$go_root")"
  {
    printf '%s\n' '#!/bin/sh'
    printf '%s\n' 'set -u'
    # Keep the black-box executor isolated from parent-runner metadata that
    # could alter tool selection or inject unrelated execution context. Keep
    # only the OS, Codex home, proxy, locale, and explicit provider credentials
    # needed by a standalone local Codex process.
    printf '%s\n' 'codex_env() {'
    if [ -n "$go_root" ]; then
      printf '  env -i "HOME=${HOME:-}" "PATH=${PATH:-/usr/bin:/bin}" "CODEX_HOME=${CODEX_HOME:-}" "TMPDIR=${TMPDIR:-/tmp}" "LANG=${LANG:-}" "LC_ALL=${LC_ALL:-}" "TERM=${TERM:-}" "HTTP_PROXY=${HTTP_PROXY:-}" "HTTPS_PROXY=${HTTPS_PROXY:-}" "ALL_PROXY=${ALL_PROXY:-}" "NO_PROXY=${NO_PROXY:-}" "SSL_CERT_FILE=${SSL_CERT_FILE:-}" "REQUESTS_CA_BUNDLE=${REQUESTS_CA_BUNDLE:-}" "OPENAI_API_KEY=${OPENAI_API_KEY:-}" "CODEX_API_KEY=${CODEX_API_KEY:-}" "GOROOT=%s" "$@"\n' "$go_root"
    else
      printf '%s\n' '  env -i "HOME=${HOME:-}" "PATH=${PATH:-/usr/bin:/bin}" "CODEX_HOME=${CODEX_HOME:-}" "TMPDIR=${TMPDIR:-/tmp}" "LANG=${LANG:-}" "LC_ALL=${LC_ALL:-}" "TERM=${TERM:-}" "HTTP_PROXY=${HTTP_PROXY:-}" "HTTPS_PROXY=${HTTPS_PROXY:-}" "ALL_PROXY=${ALL_PROXY:-}" "NO_PROXY=${NO_PROXY:-}" "SSL_CERT_FILE=${SSL_CERT_FILE:-}" "REQUESTS_CA_BUNDLE=${REQUESTS_CA_BUNDLE:-}" "OPENAI_API_KEY=${OPENAI_API_KEY:-}" "CODEX_API_KEY=${CODEX_API_KEY:-}" "$@"'
    fi
    printf '%s\n' '}'
    # app-server speaks JSON-RPC on stdin and must not be wrapped in the
    # one-shot prompt buffering/retry protocol below.
    printf '%s\n' 'if [ "${1:-}" = app-server ]; then codex_env '"$executor_quoted"' "$@"; exit $?; fi'
