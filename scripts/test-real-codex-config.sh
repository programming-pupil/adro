...
first_table_line="$(grep -n '^\[' "$run_root/run/config.toml" | head -1 | cut -d: -f1)"
sandbox_line="$(grep -n '^sandbox_mode = "danger-full-access"$' "$run_root/run/config.toml" | cut -d: -f1)"
[ -n "$sandbox_line" ] && [ "$sandbox_line" -lt "$first_table_line" ]
[ "$(sed -n '1p' "$run_root/run/config.toml")" = '# BEGIN adro-managed (do not edit; regenerated by real runner)' ]
model_line="$(grep -n '^model = "gpt-test"$' "$run_root/run/config.toml" | cut -d: -f1)"
[ -n "$model_line" ] && [ "$model_line" -lt "$first_table_line" ]
...
first_table_line="$(grep -n '^\[' "$run_root/bearer-run/config.toml" | head -1 | cut -d: -f1)"
sandbox_line="$(grep -n '^sandbox_mode = "danger-full-access"$' "$run_root/bearer-run/config.toml" | cut -d: -f1)"
[ -n "$sandbox_line" ] && [ "$sandbox_line" -lt "$first_table_line" ]
provider_line="$(grep -n '^model_provider = "custom"$' "$run_root/bearer-run/config.toml" | cut -d: -f1)"
[ -n "$provider_line" ] && [ "$provider_line" -lt "$first_table_line" ]
